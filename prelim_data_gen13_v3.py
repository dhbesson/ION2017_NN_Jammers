...
P_j = map(add,P_j, -50. + 100*np.random.rand(np.size(P_j)))
...
temp_zz_j = map(add,temp_yy_j, -3. + 6*np.random.rand(np.size(temp_zz_j)))
...
xx = map(add,xx, -2. + 4*np.random.rand(np.size(xx)))
yy = map(add,yy, -2. + 4*np.random.rand(np.size(xx)))
zz = map(add,zz, -2. + 4*np.random.rand(np.size(zz)))
