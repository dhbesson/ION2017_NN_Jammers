...
x_j = np.linspace(100, 6900, nx)
y_j = np.linspace(100, 4650, ny)
...
np.random.seed(2)
test_jammer_choices = np.random.choice(max_jammers, number_training_jammers, replace=False)
...
z_sensor = np.linspace(60, 60, nz_sensor)
...
np.random.seed(4)
test_jammer_combos_choices = np.random.choice(xrange(combo_1+combo_2,num_combo), size_training_set, replace=False)
test_jammer_combos_choices = np.append(test_jammer_combos_choices,xrange(0,combo_1+combo_2))
...
for i, combo in enumerate(jammer_combos):
    ...
    if (i % (5 * point) == 0):
        ...
        sys.stdout.flush()
print(jam_test['jam_combo_coords'][12000])
...
np.save('final_test_data.npy', final_test)
np.save('macro_params.npy', macro_params)
...
[ax.scatter(i_jam[0],i_jam[1], s=50) for i_jam in new_temp_jam_coords]
[ax.scatter(i_jam[0],i_jam[1], color='y',s=20) for i_jam in test_temp_jam_coords]
...
ax.scatter(jam_test['sensor_coords'][:,0],jam_test['sensor_coords'][:,1], color='r', s=100)
plt.show()
